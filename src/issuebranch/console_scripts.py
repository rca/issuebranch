...
ISSUE_BRANCH_FORMAT = re.compile(
    r"(?P<changetype>[^/]+)/(?P<issue_prefix>[-a-z]*)(?P<issue_number>\d+)-(?P<slug>.*)"
)
...
def issue_branch():
    ...
    parser.add_argument(
        ...
        "--pr",
        action="store_true",
        help="open a pull request seeded with an empty commit",
    )
    parser.add_argument(
        "--upstream",
        "-u",
        help="the remote to push the branch for creating pull requests",
    )
    parser.add_argument(
        "--no-resolves",
        action="store_false",
    ...
    parser.add_argument(
        "issue_number", nargs="?", help="the issue tracker's issue number"
    )
...
def make_issue_branch(args, issue, issue_number=None):
    ...
    if not prefix:
        try:
            ...
            changetypes = sorted(
                [x for x in issue.get_labels() if x["name"].startswith("changetype:")],
                key=lambda x: x["name"],
            )
...
def issue_column(argv=None):
    ...
    parser.add_argument("project", help="the project name")
    parser.add_argument(
        "issue_number", type=int, help="the issue tracker's issue number"
    )
    parser.add_argument("column", help="the name of the column to move the issue to")
    parser.add_argument(
        "--position", help="location of the card; can be 'top' or 'bottom'"
    )
...
def projects():
    ...
    copy_column_parser.add_argument(
        "kanban_board", help="the kanban board to put the cards into"
    )
    ...
    clone_parser.add_argument(
        "--no-cards", action="store_false", dest="cards", help="do not clone cards"
    )
    ...
    columns_parser.add_argument(
        "--action",
        action="store_const",
        const=projects_columns_print,
        default=projects_columns_print,
        help="print the columns",
    )
    columns_parser.add_argument(
        "--clear",
        action="store_const",
        const=projects_columns_clear,
        dest="action",
        help="clear the column in the specified project",
    )
    columns_parser.add_argument(
        "--verbose", "-v", action="store_true", help="show all json"
    )
