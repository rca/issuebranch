...
class UtilsTestCase(TestCase):
    def test_get_issue_number_from_card_data(self, *mocks):
        ...
        card_data = {
            ...
            'creator': {
                'login': 'marcusianlevine',
                'lots_of_other_keys': 'removed',
            },
    ...
    def test_get_label(self, *mocks):
        ...
        """
        self.assertEqual('team_core_engineering', utils.get_label('TEAM - Core Engineering'))

    def test_get_label_with_prefix(self, *mocks):
        """
        Ensures the label is properly parsed
        """
        self.assertEqual('team:core_engineering', utils.get_label('TEAM - Core Engineering', prefix='team'))
